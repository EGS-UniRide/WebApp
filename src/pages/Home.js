...
class Home extends Component {
  ...
  componentDidMount() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      var url_driver =
        ...
        driversPos[i].longitude +
        "," +
        driversPos[i].latitude +
        ".json?key=6yDldySYut6Xgsl7SNwD&limit=1";

      axios.get(url_driver).then((response) => {
  ...
  changeSimAnimal = (event) => {
    if (
      ...
      document.getElementById("nao-animal").checked === true
    ) {
      swal(
        "Operação não permitida!",
        "Apenas é possível selecionar uma resposta!",
        "error"
      );
  ...
  changeNaoAnimal = (event) => {
    if (
      event.target.checked === true &&
      document.getElementById("sim-animal").checked === true
    ) {
      swal(
        "Operação não permitida!",
        "Apenas é possível selecionar uma resposta!",
  ...
  changeSimCrianca = (event) => {
    if (
      event.target.checked === true &&
      document.getElementById("nao-crianca").checked === true
    ) {
      swal(
        "Operação não permitida!",
        "Apenas é possível selecionar uma resposta!",
  ...
  changeNaoCrianca = (event) => {
    if (
      event.target.checked === true &&
      document.getElementById("sim-crianca").checked === true
    ) {
      swal(
        "Operação não permitida!",
        "Apenas é possível selecionar uma resposta!",
  ...
  changeSimBagagem = (event) => {
    if (
      event.target.checked === true &&
      document.getElementById("nao-bagagem").checked === true
    ) {
      swal(
        "Operação não permitida!",
        "Apenas é possível selecionar uma resposta!",
  ...
  changeNaoBagagem = (event) => {
    if (
      event.target.checked === true &&
      document.getElementById("sim-bagagem").checked === true
    ) {
      swal(
        "Operação não permitida!",
        "Apenas é possível selecionar uma resposta!",
  ...
  startTravel = () => {
    ...
    if (checksNums !== 3) {
      swal(
        "Respostas Inválidas!",
        "É necessário responder a todas as questões colocadas!",
        "error"
      );
    } else {
      ...
      axios
        .post("http://apiuniride.egs/matching/objects/v1/match", matchPayload)
        .then((response) => {
          ...
          response.data.forEach((result, index) => {
            data.drivers.forEach((driver, index) => {
              if (
                result["a1"] == driver.questions[0]["a1"] &&
                result["a2"] == driver.questions[0]["a2"] &&
          ...
          axios
            .post(
              "http://apiuniride.egs/closer/points/v1/inside-points?latitude=" +
                this.state.passenger[0] +
                "&longitude=" +
                this.state.passenger[1] +
                "&range=1",
              pointsPayload
            )
            .then((response) => {
              ...
              if (response.data.length === 0) {
                axios
                  ...
                  .then((response) => {
                    ...
                    data.drivers.forEach((driver, index) => {
                      if (
                        response.data[randomIndex]["latitude"] ===
                          driver.latitude &&
                        response.data[randomIndex]["longitude"] ===
                          driver.longitude
                      ) {
                        // console.log(driver.id)
                        ...
                        axios
                          .post(
                            "http://localhost:8000/v2/payments/create-checkout-session/price_1MkrnbA6JRbT2gXKP8tazKO8"
                          )
                          .then((response) => {
                            ...
                            axios
                              .post(
                                "http://apiuniride.egs/payments/v2/payments/bill",
                                {
                                  payerid: payerid,
                              ...
                              .then((response) => {
                                if (response["status"] === 200) {
                                  axios.post(
                                    "http://notificationsapi-svc:8010/v2/email",
                                    {
                                      address: "tahd99@gmail.com",
                                      subject: "UniRide Payments",
                                      description:
                                        "You owe 3.20eur for your last ride",
                                    }
                                  );
                  ...
                  if (
                    ...
                    axios
                      ...
                      .then((response) => {
                        ...
                        window.location.href = url;
                        axios
                          .post("http://apiuniride.egs/v2/payments/bill", {
                            payerid: payerid,
                            receiverid: driver.id,
                            paydate: new Date(Date.now()),
                          })
  ...
  test = () => {
    // Call the MatchingAPI
    axios.get("http://apiuniride.egs/v2/payments/bills").then((response) => {
      console.log(response.data);
    });
  };
  ...
  render() {
    ...
    var carIcon = new Icon({
      iconUrl:
        "https://cdn.pixabay.com/photo/2022/06/06/14/39/marker-7246182_960_720.png",
      shadowUrl:
      ...
      <div style={{ backgroundColor: "#addbd0", paddingRight: "0px" }}>
        ...
        >
              ...
              <Row style={{ marginLeft: "4%" }}>
                ...
                <div style={{ marginLeft: "3.4%", marginBottom: "5%" }}>
                  <input
                    ...
                    value="Sim"
                    id="sim-crianca"
                    onChange={this.changeSimCrianca}
                  />{" "}
                  Sim
                  <input
                    type="checkbox"
                    value="Não"
                    id="nao-crianca"
                    onChange={this.changeNaoCrianca}
                    style={{ marginLeft: "5%" }}
                  />{" "}
                  Não
                </div>
              ...
              <Row style={{ marginLeft: "4%" }}>
                ...
                <div style={{ marginLeft: "3.4%" }}>
                  <input
                    ...
                    value="Sim"
                    id="sim-bagagem"
                    onChange={this.changeSimBagagem}
                  />{" "}
                  Sim
                  <input
                    type="checkbox"
                    value="Não"
                    id="nao-bagagem"
                    onChange={this.changeNaoBagagem}
                    style={{ marginLeft: "5%" }}
                  />{" "}
                  Não
                </div>
          ...
          <Modal.Footer>
            <div style={{ display: "flex", justifyContent: "right" }}>
              <Button
                variant="primary"
                onClick={this.startTravel}
                style={{
                  backgroundColor: "#fcc186",
                  border: "none",
                  width: "130px",
                  color: "black",
                }}
        ...
        <Container fluid style={{ paddingLeft: "0px", paddingRight: "0px" }}>
          ...
          >
            ...
            />
            <Marker
              position={passenger[0] == undefined ? [0, 0] : passenger}
              icon={redIcon}
            ></Marker>
              ...
              <div className="car-box">
                <div className="car-image">
                  <img
                    id="profilePic"
                    src={car.imageSrc}
                    onClick={() => this.redirectDriverProfile(car.id)}
                  />
                </div>
                ...
                <div className="car-local">
                  <p>
                    {" "}
                    <strong className="bold">Local de Partida:</strong>{" "}
                    {car.localAtual}
                  </p>
                  <div
                    style={{
